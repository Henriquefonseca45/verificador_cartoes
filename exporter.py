...
def _assign_detail_boxes_to_cards(
    detail_boxes: List[fitz.Rect],
    sim_cards: List[ClassifiedCard],
    detail_page: fitz.Page,
) -> Dict[int, fitz.Rect]:
    assigned: Dict[int, fitz.Rect] = {}
    remaining = detail_boxes[:]

    for card in sim_cards:
        expected_slot = _slot_rect(_back_slot_index(card.source.card_index)) & detail_page.rect
        best_index = None
        best_score = 0.0

        for idx, box in enumerate(remaining):
            overlap = _intersection_area(box, expected_slot)
            if overlap <= 0:
                continue
            score = overlap / max(1.0, box.get_area())
            if score > best_score:
                best_score = score
                best_index = idx

        if best_index is not None and best_score >= 0.20:
            assigned[card.source.card_index] = remaining.pop(best_index)
...
def _build_detail_crop_map(all_grouped_cards: Dict[str, List[ClassifiedCard]]) -> Dict[Tuple[str, int, int], fitz.Rect | None]:
    ...
    try:
        for (pdf_path, front_page_number), page_cards in by_front_page.items():
            ...
            slot_assigned = _assign_detail_boxes_to_cards(detail_boxes, sim_cards, detail_page)
            for card in sim_cards:
                box = slot_assigned.get(card.source.card_index)
                if box is not None:
                    crop_map[(pdf_path, front_page_number, card.source.card_index)] = _expand_rect(box, detail_page.rect)

            missing_cards = [c for c in sim_cards if c.source.card_index not in slot_assigned]
            missing_keys = [(pdf_path, front_page_number, c.source.card_index) for c in missing_cards]
            if not missing_cards:
                continue

            unassigned_boxes = [
                box for box in detail_boxes
                if all(box != assigned_box for assigned_box in slot_assigned.values())
            ]

            if len(unassigned_boxes) >= len(missing_cards):
                for key, box in zip(missing_keys, unassigned_boxes[: len(missing_cards)]):
                    crop_map[key] = _expand_rect(box, detail_page.rect)
            elif unassigned_boxes:
                ...
                unassigned_boxes = sorted(unassigned_boxes, key=lambda r: r.get_area(), reverse=True)
                for key, box in zip(missing_keys, unassigned_boxes):
                    crop_map[key] = _expand_rect(box, detail_page.rect)
            else:
